...
description: "Master prompts and session kickoff templates for Spec Driven Development (SDD): Mode 1 (New feature from PRD), Mode 2 (Brownfield changes with proposal deltas), Mode 3 (Greenfield setup), and Session Resume checkpoints."
...
**Passo 5 — Modelos de Raciocínio Analítico e Criação de tasks**
Antes de redigir a lista de tarefas, formule explicitamente a seção de **Modelos de Raciocínio Analítico** (Primeiros Princípios, Premortem/Inversão de modos de falha, Validação MECE, Árvore de Decisão/Poda de alternativas, Falsificabilidade/TDD e Conformidade Constitucional).
Em seguida, crie `openspec/changes/[id]/tasks.md` com tarefas específicas, executáveis e testáveis.
...
Antes de listar as tarefas, aplique os **Modelos de Raciocínio Analítico** (em especial Inversão/Premortem de regressões e validação MECE do delta).
...
Siga o fluxo: spec → plan → modelos de raciocínio analítico → tasks → implementação.

**Regra para este projeto:**
A cada nova feature, sempre criar spec → plan → modelos de raciocínio analítico → tasks antes de qualquer código.
```
