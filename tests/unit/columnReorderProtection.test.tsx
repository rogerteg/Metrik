...
describe('Column Reorder Rules (Fixed Ends Protection)', () => {
  ...
  describe('canMoveColumn Validator', () => {
    it('rejects any move if board has 2 or fewer columns', () => {
      expect(canMoveColumn(2, 0, 1)).toBe(false);
    ...
    it('STRICTLY rejects moving the first column (index 0 / To Do)', () => {
      ...
      expect(canMoveColumn(5, 0, 3)).toBe(false);
    });
    ...
    it('STRICTLY rejects moving any column TO the first column position (targetIndex 0)', () => {
      ...
      expect(canMoveColumn(5, 3, 0)).toBe(false);
    });

    it('STRICTLY rejects moving the last column (index length - 1 / Completed)', () => {
      expect(canMoveColumn(5, 4, 3)).toBe(false);
      expect(canMoveColumn(5, 4, 2)).toBe(false);
      expect(canMoveColumn(5, 4, 1)).toBe(false);
    });

    it('STRICTLY rejects moving any column TO the last column position (targetIndex length - 1)', () => {
      expect(canMoveColumn(5, 1, 4)).toBe(false);
      expect(canMoveColumn(5, 2, 4)).toBe(false);
      expect(canMoveColumn(5, 3, 4)).toBe(false);
    });

    it('allows moving intermediate columns between each other', () => {
      // Columns at indices 1, 2, 3 can be moved freely among indices 1, 2, 3
      expect(canMoveColumn(5, 1, 2)).toBe(true);
      expect(canMoveColumn(5, 2, 1)).toBe(true);
      expect(canMoveColumn(5, 2, 3)).toBe(true);
      expect(canMoveColumn(5, 3, 2)).toBe(true);
      expect(canMoveColumn(5, 1, 3)).toBe(true);
      expect(canMoveColumn(5, 3, 1)).toBe(true);
    });
  });
  ...
  describe('reorderColumnList Helper', () => {
    ...
    it('returns original list if movement violates last column lock', () => {
      const result = reorderColumnList(sampleColumns, 4, 2);
      expect(result).toBe(sampleColumns);
      expect(result[4].id).toBe('completed');
    });

    it('successfully swaps intermediate columns while keeping To Do and Completed untouched', () => {
      // Move 'dev' (index 1) to index 2
      const result = reorderColumnList(sampleColumns, 1, 2);
      expect(result).not.toBe(sampleColumns);
      expect(result[0].id).toBe('todo'); // First stays untouched
      expect(result[1].id).toBe('review');
      expect(result[2].id).toBe('dev');
      expect(result[3].id).toBe('qa');
      expect(result[4].id).toBe('completed'); // Last stays untouched
    });
  });
  ...
  describe('Column Component UI: Locks and Buttons', () => {
    ...
    it('renders lock indicator and NO move buttons on the last column (Completed)', () => {
      render(
        <Column
          column={sampleColumns[4]}
          count={5}
          columnIndex={4}
          totalColumns={5}
          onMoveColumn={vi.fn()}
      ...
      expect(screen.getByLabelText('Coluna fixa')).toBeInTheDocument();
      expect(screen.queryByLabelText(/Mover coluna Completed/i)).not.toBeInTheDocument();
    });

    it('renders move buttons and drag handle for intermediate columns with boundary disablement', () => {
    ...
    it('disables right button for intermediate column adjacent to the last column', () => {
      const mockMove = vi.fn();
      render(
        <Column
      ...
      expect(rightBtn).toBeDisabled(); // Cannot move right into index 4

      fireEvent.click(leftBtn);
      expect(mockMove).toHaveBeenCalledWith(3, 2);
    });

    it('handles column drag-and-drop between intermediate columns', () => {
      const mockMove = vi.fn();
      render(
        <Column
