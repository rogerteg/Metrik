...
description: "Spec Driven Development (SDD) — guia completo da metodologia spec-first: artefatos vivos (constitution.md, project-context.md, lessons-learned.md, specs, changes), fases Especificar → Planejar → Tarefas → Implementar, pirâmide de testes, constitution, checklist de PR, anti-padrões e boas práticas por stack (front-end, back-end, APIs REST, banco). Use ao criar ou revisar specs, planos e tarefas, ou ao implementar features seguindo SDD."
compatibility: "Google Antigravity (AGY) · Claude Code · Trae IDE · VS Code + Codex · Cursor · Windsurf · OpenCode · Kilo Code"
metadata:
